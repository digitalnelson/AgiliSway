...
using AgiliSway.vNext.Subjects;
using AgiliSway.vNext.Collections;
using AgiliSway.vNext.Studies;
using Ninject;
using AgiliSway.vNext.Event;
using AgiliSway.vNext.Dialogs;
using AgiliSway.vNext.Services;
using AgiliSway.vNext.Flyouts;
...
namespace AgiliSway.vNext
{
	public class ShellViewModel :  Conductor<IScreen>.Collection.OneActive, IShell, IHandle<DataChangedEvent>, IHandle<StudySelectedEvent>, IHandle<SubjectSelectedEvent>
	{
			...
			IDbStorage localStorage, IEventAggregator events, IWindowManager windowManager, IAppPreferences appPreferences, IDeviceManager deviceManager)
		{
			...
			Items.Add(IoC.Get<AgiliSway.vNext.Sections.Device.MainViewModel>());
