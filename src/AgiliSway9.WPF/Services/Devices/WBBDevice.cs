...
namespace AgiliSway9.WPF.Services.Devices
{
	public class WBBDevice : IDevice
	{
        ...
        public async Task<Models.CollectionDataSet> Collect(TimeSpan time, IProgress<Models.CollectProgress> progress, System.Threading.CancellationToken ct, CollectionDataPoint calibration)
		{
            ...
            while (elapsed < time && !ct.IsCancellationRequested)
            {
                ...
                if (lastPoint != null)
                {
                    try
                    {
                        AgiliSway9.WPF.Models.Point cop = null;
                        if (calibration != null)
                        {
        ...
        public AgiliSway9.WPF.Models.Point COP(int topLeft, int topRight, int bottomLeft, int bottomRight)
        {
            AgiliSway9.WPF.Models.Point pt = new AgiliSway9.WPF.Models.Point
            {
                X = 0,
                Y = 0
            };
